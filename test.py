...
def apply_constraints():
  s1.assign(tf.maximum(s1, 1e-05)) # ensure the sigmas are positive (greater or equal to some small
  s2.assign(tf.maximum(s2, 1e-05))            # positive number, e.g. 1e-5) and alpha is in (0, 1) interval
  alpha.assign(tf.maximum(0., tf.minimum(1., alpha)))            # hint: use the `assign` method to update the variable values
...
for i in range(30):
  ...
  for v, g in zip(variables, grads):
    v.assign_add(learning_rate * g) # Apply the gradients manually (add
